...
Please be cautious. Working with floating point intervals is much slower than standard floating point operations, and should only be used where the added guarantee of 'could be equal' is required. In addition, since the package is still under active development and has not been uploaded to PyPi yet, installing the package is a bit of a pain right now. In case you want to try it out on your own computer you need to:
...
```
3. Build the project. This requires a working c99 compliant c compiler. If this works (and that's a big IF), then it will build a wheel file in the dist folder.
```sh
(.venv) > python -m build
```
